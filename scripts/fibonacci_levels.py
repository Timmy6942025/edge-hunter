...
def main():
    ...
    recent_low = min(l['price'] for l in lows[-3:]) if len(lows) >= 3 else lows[-1]['price']
    ...
    support_levels = [l for l in levels if l['type'] == 'Retracement' and l['price'] < current_price]
    resistance_levels = [l for l in levels if l['type'] == 'Retracement' and l['price'] > current_price]
    
    if support_levels:
        print(f'\n   🟢 SUPPORT LEVELS:')
        for l in sorted(support_levels, key=lambda x: abs(x['price'] - current_price))[:4]:
            strength = 'STRONG' if l['fib'] in [0.618, 0.382] else ('MODERATE' if l['fib'] == 0.5 else 'WEAK')
            print(f'      {l["fib"]:.3f}: ${l["price"]:.2f} ({strength})')
    
    if resistance_levels:
        print(f'\n   🔴 RESISTANCE LEVELS:')
        for l in sorted(resistance_levels, key=lambda x: abs(x['price'] - current_price))[:4]:
            strength = 'STRONG' if l['fib'] in [0.618, 0.382] else ('MODERATE' if l['fib'] == 0.5 else 'WEAK')
            print(f'      {l["fib"]:.3f}: ${l["price"]:.2f} ({strength})')
    ...
    fib_382 = [l['price'] for l in levels if l['fib'] == 0.382 and l['type'] == 'Retracement']
    fib_618 = [l['price'] for l in levels if l['fib'] == 0.618 and l['type'] == 'Retracement']
