...
def analyze_ticker(ticker, period='1y'):
    ...
    try:
        ...
        # Calculate returns
        returns_1d = ((safe_float(data['Close'].iloc[-1]) / safe_float(data['Close'].iloc[-2])) - 1) * 100 if len(data) > 1 else 0
        returns_1w = ((safe_float(data['Close'].iloc[-1]) / safe_float(data['Close'].iloc[-6])) - 1) * 100 if len(data) > 5 else 0
        returns_1m = ((safe_float(data['Close'].iloc[-1]) / safe_float(data['Close'].iloc[-22])) - 1) * 100 if len(data) > 21 else 0
...
def main():
    ...
    for r in sorted(results, key=lambda x: x['returns_1y'], reverse=True):
        ...
        print(f'│{r["ticker"]:<8} ${r["current"]:<7.2f} {r["returns_1d"]:>+6.1f}%{emoji_1d} {r["returns_1w"]:>+6.1f}% {r["returns_1m"]:>+6.1f}% {r["returns_3m"]:>+6.1f}% {r["returns_1y"]:>+6.1f}%│')
    print(f'└{"─"*68+"┘"}')
    ...
    for r in sorted(results, key=lambda x: x['score'], reverse=True):
        print(f'│{r["ticker"]:<8} {r["volatility_30d"]:>9.1f}% {r["volatility_1y"]:>9.1f}% {r["max_drawdown"]:>9.1f}% {r["sharpe"]:>7.2f} {r["score"]:>6.1f}│')
    print(f'└{"─"*68+"┘"}')
