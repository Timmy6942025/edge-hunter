...
def detect_regime(data, short_window=20, long_window=50):
    ...
    volatility_50d = (
        returns.tail(50).std() * np.sqrt(252)
        if len(returns) >= 50 else volatility_20d
    )

    # Trend strength
    trend_50d = (
        float(((data['Close'].iloc[-1] / data['Close'].iloc[-50]) - 1) * 100)
    ...
    ma_slope = (
        ...
        if len(ma_short) > 5 and ma_short.iloc[-5] != 0 else 0
    )
